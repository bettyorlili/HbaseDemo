...
public class HbaseApi {
    ...
    public void init() throws Exception {
        ...
        conf.set("hbase.zookeeper.quorum","192.168.241.129");// zookeeper 地址
         conf.set("hbase.zookeeper.property.clientPort","2181");// zookeeper 客户端端口，默认为2188，可以不用设置
        conn = ConnectionFactory.createConnection(conf);// 创建连接

        admin = (HBaseAdmin) conn.getAdmin(); // hbase 表管理类

        tableName = TableName.valueOf("studentsapi"); // 表名
